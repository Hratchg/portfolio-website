...
These packages need to be added to `package.json`:

- `bcryptjs` — password hashing for admin auth
- `express-rate-limit` — rate limiting on login endpoint
...
- The existing `users` table in `shared/schema.ts` and associated `IStorage` methods (`getUser`, `getUserByUsername`, `createUser`) will be removed. They are unused — the app has no user registration flow. The `admin_user` table replaces them for admin auth.
- The existing `passport` and `passport-local` dependencies will be removed. Auth will use a simple custom middleware with `express-session` directly, since there's only one user and one auth strategy.
...
- For items lacking `id` fields in `portfolio.ts` (skills, nav links), the seed script generates UUIDs at insert time.
- For items lacking `sort_order`, the seed script assigns sort order based on array index position.
- The `admin_user` row is seeded from the `ADMIN_USERNAME` and `ADMIN_PASSWORD_HASH` environment variables on first run.
...
4. On success: session cookie set (session flag marks user as authenticated), redirect to `/`
5. On failure: error message, rate-limited (5 attempts per minute via `express-rate-limit`)
...
- Admin credentials are stored in the `admin_user` database table (single row).
- On first deploy, the seed script creates the row from `ADMIN_USERNAME` and `ADMIN_PASSWORD_HASH` env vars.
- The login endpoint validates against the database table, not env vars directly. This allows password changes without redeploying.
- The env vars are only used for initial seeding.

### Security

- bcryptjs password hashing
- Rate limiting on login endpoint via `express-rate-limit` (5 attempts/minute)
- Session cookie with `httpOnly`, `secure` (in production), `sameSite: strict`
- All protected API routes check session via auth middleware before processing
...
3. Click Save → PUT to `/api/content/batch` with the full changeset
...
| `ADMIN_USERNAME` | Login username (used only for initial seed) |
| `ADMIN_PASSWORD_HASH` | bcrypt hash of login password (used only for initial seed) |
| `NODE_ENV` | `production` |
...
- Remove `passport`, `passport-local`, `memorystore`, `ws` dependencies (and their `@types/` packages)
...
### Editing on the Experience Page

The experience page currently renders experiences, education, featured projects, and skills all on one page. In edit mode, all of these sections are editable inline on this page — projects and skills shown here are the same data as would appear elsewhere. Edits to a project on the experience page update the same database row.
...
server/
  ...
  storage.ts            (modified - PostgreSQL implementation, remove old users/IStorage)
  auth.ts               (new - auth middleware, login/logout handlers, rate limiting)
  seed.ts               (new - database seed script)
shared/
  schema.ts             (modified - add all content table schemas, remove old users table)
...
client/src/
  ...
  components/
    ...
    navbar.tsx           (modified - use API data for navLinks + personalInfo)
    footer.tsx           (modified - use API data for personalInfo)
    contact-section.tsx  (modified - use API data for personalInfo)
  pages/
